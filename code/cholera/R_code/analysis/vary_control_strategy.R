...
movement_params <- expand.grid(m1 = c(0,0.01, 0.005), 
                               ...
                               control_type = c("unique", "uniform"),
                               experiment_type = "movement")
test_params <- test_param_builder(movement_params, all_params)
# define disease parameters to sweep across (+/- 10% from baseline parameter)
transmission_params <- expand.grid(beta_W1 = params$beta_W1*c(0.9,1,1.1), 
                                   beta_W2 = params$beta_W2*c(0.9,1,1.1),
                                   control_type = c("unique", "uniform"),
                                   experiment_type = "transmission")
...
env_params <- expand.grid(rho1 = params$rho1*c(0.9,1,1.1), 
...
cost_params <- expand.grid(b2 = oc_params$b2*c(0.9,1,1.1),
...
v_states <- select(exper$states, -S1, -S2, -I1, -I2, -R1, -R2, -W1, -W2) # will streamline this later

mult_oc_params <- melt(v_states %>% select(-test_case),
