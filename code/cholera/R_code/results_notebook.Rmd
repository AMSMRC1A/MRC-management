...
library(doParallel)
library(foreach)
registerDoParallel(detectCores() - 2) # update this 4 if you want to use more cores
```
...
params_table<- matrix(as.list(params),1, length(as.list(params)))
colnames(params_table) <- names(as.list(params))

oc_params_table<- matrix(as.list(oc_params),1, length(as.list(oc_params)))
colnames(oc_params_table) <- names(as.list(oc_params))

IC_table<- matrix(as.list(IC),1, length(as.list(IC)))
colnames(IC_table) <- names(as.list(IC))
...
Parameters: `r knitr::kable(params_table, format="markdown")`

Optimal control parameters: `r knitr::kable(oc_params_table, format = "markdown")`

Initial conditions: `r knitr::kable(IC_table, format =  "markdown")`

### No control
Black line indicates the start of controls in other scenarios at day `r response_time`.
```{r no-control}
...
uncontrolled_epi %>%
  ...
  geom_line() +
  geom_vline(aes(xintercept = response_time)) +
...
# reformat output
exper_v_only <- reformat_mult_params_output(
  output = exper_v_only,
  test_params = test_params
)
```

```{r vacc_only_plot, fig.show="hold", out.width = "100%"}
# plot states
exper_v_only$v %>%
...
# reformat output
exper_u_only <- reformat_mult_params_output(
  output = exper_u_only,
  test_params = test_params
)
```

```{r sani_only_plot, fig.show="hold", out.width = "100%"}
# plot states
exper_u_only$v %>%
...
# reformat output
exper_both <- reformat_mult_params_output(
  output = exper_both,
  test_params = test_params
)
```
