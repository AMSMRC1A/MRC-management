...
test_params <- expand.grid(
  mu1 = c(0, 0.01), # note changed this
...
l <- length(new_times)
plot_df <- lapply(1:length(out), function(i) {
  cbind(sim = rep(i, l), out[[i]][, c("time", "I1", "I2")])
})
plot_df <- do.call(rbind, plot_df)
# add parameters of interest
plot_df <- left_join(
  test_params %>% select(mu1, m1, n1, rho1, m2, n2, rho2, sim),
